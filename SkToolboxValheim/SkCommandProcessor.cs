  ...
  internal static class SkCommandProcessor {
    ...
    public static void InitCommands() {
      ...
      if (Console.instance != null) {
        ...
        //    }
        //}
        new Terminal.ConsoleCommand("gotoxyz", "[x z] or [x z y] - Teleport by ComfyToolbox", delegate (Terminal.ConsoleEventArgs args) {
          int x;
          int z;
          int y;
          if (args.Length < 3 || !args.TryParameterInt(1, out x) || !args.TryParameterInt(2, out z) || Player.m_localPlayer == null) {
            return false;
          }
          Vector3 vector = new Vector3((float)x, Player.m_localPlayer.transform.position.y, (float)z);
          if (!args.TryParameterInt(3, out y)) {
            float num3 = (Player.m_localPlayer.IsDebugFlying() ? 400f : ZoneSystem.instance.m_waterLevel);
            vector.y = Mathf.Clamp(vector.y, ZoneSystem.instance.m_waterLevel, num3);
          } else {
            vector.y = y;
          }

          Player.m_localPlayer.TeleportTo(vector, Player.m_localPlayer.transform.rotation, true);
